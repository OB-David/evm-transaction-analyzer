...
def get_contract_color(contract_addr: str, contract_colors: List[str]) -> str:
    """按合约地址哈希 → 固定颜色"""
    if not contract_addr or contract_addr.strip() == "" or contract_addr == "Unknown":
        return "#FFFFFF"

    addr_bytes = contract_addr.strip().lower().encode('utf-8')
    hash_obj = hashlib.md5(addr_bytes)
    hash_int = int(hash_obj.hexdigest(), 16)
    color_idx = hash_int % len(contract_colors)
    return contract_colors[color_idx]
...
def render_transaction(cfg: object, output_path: str, full_address_name_map: Dict[str, str], rankdir: str = "TB") -> None:
    ...
    # 获取有效节点、颜色、合约地址映射
    valid_nodes, node_colors, node_contract_addrs, contract_to_color_idx = get_valid_nodes_and_colors(cfg, contract_colors)
    if not valid_nodes:
        print("警告：无有效节点可渲染")
        return
